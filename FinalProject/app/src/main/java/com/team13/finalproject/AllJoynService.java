...
public class AllJoynService extends Service implements Observer {
     ...
     * this obejct.
     */
    private BusAttachment mBus  = new BusAttachment(WiForum.PACKAGE_NAME, BusAttachment.RemoteMessage.Receive);
    ...
    private class ChatBusListener extends BusListener {
        ...
        public void foundAdvertisedName(String name, short transport, String namePrefix) {
            ...
            WiForum application = (WiForum)getApplication();
    ...
    private void doConnect() {
        ...
        if (Status.OK != status) {
            //wiforum.alljoynError(WiForum.Module.HOST, "Unable to register the chat bus object: (" + status + ")");
            return;
        }

        status = mBus.connect();
        if (status != Status.OK) {
            //wiforum.alljoynError(ChatApplication.Module.GENERAL, "Unable to connect to the bus: (" + status + ")");
            return;
        }
    ...
    private void doRequestName() {
         ...
         */
        String wellKnownName = NAME_PREFIX + "." + wiforum.hostGetChannelName();
        ...
        if (status == Status.OK) {
            ...
            wiforum.hostSetChannelState(mHostChannelState);
        } else {
    ...
    private void doBindSession() {
        ...
        Status status = mBus.bindSessionPort(contactPort, sessionOpts, new SessionPortListener() {
            ...
            public void sessionJoined(short sessionPort, int id, String joiner) {
                ...
                mHostChatInterface = emitter.getInterface(WiForumInterface.class);
            }
        });
        ...
        if (status == Status.OK) {
            ...
            //wiforum.alljoynError(WiForum.Module.HOST, "Unable to bind session contact port: (" + status + ")");
            return;
        }
    }
     ...
     * description of this rather non-intuitively complicated case.
     */
    WiForumInterface mHostChatInterface = null;
    ...
    private void doAdvertise() {
        ...
        if (status == Status.OK) {
           ...
           //wiforum.alljoynError(WiForum.Module.HOST, "Unable to advertise well-known name: (" + status + ")");
            return;
        }
    }
    ...
    private void doJoinSession() {
        ...
        if (mHostChannelState != HostChannelState.IDLE) {
            if (wiforum.useGetChannelName().equals(wiforum.hostGetChannelName())) {
                mUseChannelState = UseChannelState.JOINED;
                wiforum.useSetChannelState(mUseChannelState);
                mJoinedToSelf = true;
                return;
            }
        }
         ...
         * a reasonable name.
         */
        String wellKnownName = NAME_PREFIX + "." + wiforum.useGetChannelName();
        ...
        Status status = mBus.joinSession(wellKnownName, contactPort, sessionId, sessionOpts, new SessionListener() {
            ...
            public void sessionLost(int sessionId, int reason) {
                ...
                //wiforum.alljoynError(ChatApplication.Module.USE, "The chat session has been lost");
        ...
        if (status == Status.OK) {
            ...
            //wiforum.alljoynError(ChatApplication.Module.USE, "Unable to join chat session: (" + status + ")");
            return;
        }
     ...
     */
    WiForumInterface mChatInterface = null;
    //WiForumInterface mHostChatInterface = null;
    /**
     * Implementation of the functionality related to joining an existing
     * remote session.
    ...
    private void doSendMessages() {
        ...
        String message;
        while ((message = wiforum.getOutboundItem()) != null) {
            ...
            try {
                ...
                //wiforum.alljoynError(WiForum.Module.USE, "Bus exception while sending message: (" + ex + ")");
            }
        }
    }
     ...
     */
    class ChatService implements WiForumInterface, BusObject {
        /**
    ...
    public void Chat(String string) {
        ...
        wiforum.newRemoteUserMessage(nickname, string);
    }
